...
When helping with proposals, reference ACTUAL inventory items and pricing from the database. Reference actual procedures by name. Be concise and direct.` + (analysisResults ? (
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const shareToken = Array.from(crypto.getRandomValues(new Uint8Array(8))).map(b => b.toString(36).padStart(2, "0")).join("").slice(0, 12);
      ...
      try {
        ...
        const qrY = PH - 60 - qrSize - 16;
        doc.addImage(qrDataUrl, "PNG", qrX, qrY, qrSize, qrSize);
        doc.setFont("helvetica","normal"); doc.setFontSize(7); doc.setTextColor(...LG);
        doc.text("Scan for details", qrX + qrSize / 2, qrY + qrSize + 10, { align: "center" });
      ...
      const proposalData = {
        ...
        devices: devices.filter(d => d.qty > 0),
        categories: categoryBreakdown,
        type_label: isVE ? "Voice Evacuation + Fire Alarm" : "Standard Fire Alarm",
      };

      const newProposal = { id: uid(), client_id: clientId, client_name: cName, proposal_type: proposalType, total, status: "draft", created_at: new Date().toISOString(), filename: fn, share_token: shareToken, proposal_data: proposalData };
